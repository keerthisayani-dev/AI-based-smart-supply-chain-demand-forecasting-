...
def _resolve_csv_path() -> Path:
    configured = _env_str("DEMANDIQ_CSV_PATH", "")
    if configured:
    ...
    dataset_preference = _env_str("DEMANDIQ_DATASET_VARIANT", "cleaned").lower()
    variant_order = {
        "features": [
            FEATURES_CITY_LEVEL_CSV_PATH,
            CLEANED_CITY_LEVEL_CSV_PATH,
            CITY_LEVEL_CSV_PATH,
            LEGACY_CSV_PATH,
        ],
        "cleaned": [
    ...
    for candidate in variant_order.get(dataset_preference, variant_order["cleaned"]):
        if candidate.exists():
            return candidate
    return CLEANED_CITY_LEVEL_CSV_PATH
...
async def app_lifespan(_: FastAPI):
    if _env_bool("DEMANDIQ_PREWARM_CACHE", False):
        threading.Thread(
