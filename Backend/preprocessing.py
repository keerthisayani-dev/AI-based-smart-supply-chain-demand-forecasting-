...
def _canonical_dataset_stem(path: Path) -> str:
    stem = path.stem
    for suffix in ("_cleaned", "_encoded"):
        if stem.endswith(suffix):
            stem = stem[: -len(suffix)]
    return stem


DATASET_STEM = _canonical_dataset_stem(INPUT_CSV_PATH)
READABLE_OUTPUT_CSV_PATH = BASE_DIR / "Dataset" / f"{DATASET_STEM}_cleaned.csv"
ENCODED_OUTPUT_CSV_PATH = BASE_DIR / "Dataset" / f"{DATASET_STEM}_encoded.csv"
...
def _next_record_id_start(series: pd.Series) -> int:
    next_num = 1
    pattern = re.compile(r"^rec-(\d+)$", re.IGNORECASE)
...
if "record_id" in df.columns:
    ...
    if missing_record_ids.any():
        start_idx = _next_record_id_start(df["record_id"])
